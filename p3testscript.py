...
def result():
    ...
    f1 = open(cwd + '/samplesolutionp3.txt', "r")
    f2 = open(cwd + '/program3solution_student_output.txt', "r")
...
def compilerunfile():
    ...
    javac = "javac " + cwd + "/TomatoesDriver.java" #compile solution file command and generate class file

    proc = subprocess.run([javac], capture_output=True, text = True, shell = True)
    

    
    java = "java TomatoesDriver"
    
    f = open('program3solution_student_output.txt', "w")
...
def setupchecker():
    ...
    if not os.path.exists(cwd + "/samplesolutionp3.txt"):
